...
//    String input2 = "U7,R6,D4,L4";

        List<int[]> _1 = parse(input1);
        List<int[]> _2 = parse(input2);
        intersection(_1, _2);
    }
    ...
    private static List<int[]> parse(String input) {
        ...
        for(String position : positions) {
            ...
            int steps = Integer.valueOf(position.substring(1));
            int[] coordinates = go(row, column, direction, steps, hasPrevious, panel);
            hasPrevious = true;
            row = coordinates[0];
    ...
    private static int[] go(int row, int column, String direction, int steps, boolean hasPrevious, List<int[]> panel) {
        panel.add(new int[] {row, column});
        switch(direction) {
            case "R":
                for (int i = column + 1; i < steps + column; i++) {
                    panel.add(new int[] {row, i});
                }
                return new int[] {row, column + steps};
            case "L":
                for (int i = column - 1; i > column - steps; i--) {
                    panel.add(new int[] {row, i});
                }
                return new int[] {row, column - steps};
            case "U":
                for (int i = row + 1; i < steps + row; i++) {
                    panel.add(new int[] {i, column});
                }
                return new int[] {row + steps, column};
            case "D":
                for (int i = row - 1; i > row - steps; i--) {
                    panel.add(new int[] {i, column});
                }
                return new int[] {row - steps, column};
        }
    ...
    private static void intersection(List<int[]> path1, List<int[]> path2) {
        int result = Integer.MAX_VALUE;
        for (int i = 0; i < path1.size(); i++) {
            for (int j = 0; j < path2.size(); j++) {
                if(path1.get(i)[0] == path2.get(j)[0] && path1.get(i)[1] == path2.get(j)[1]) {
                    int intermediate = i + j;
                    if(intermediate == 0) {
                        continue;
                    }
                    System.out.println("Find (Steps 1="+ i +", Steps 2="+ j +")");
